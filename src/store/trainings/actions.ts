...
export enum TrainingActionTypes {
    ...
    FETCH_SUCCESS = '@@trainings/FETCH_SUCCESS',
...
// FETCH actions
export const fetchTrainingSuccess = (training: ModelTraining) => action(
    TrainingActionTypes.FETCH_SUCCESS, {training}
);

export function fetchTrainingRequest(id: string): AsyncAction<Promise<ModelTraining>> {
    return ((dispatch, getState, {trainingService}) => {
        return trainingService.get(id).then(training => {
            dispatch(fetchTrainingSuccess(training));
